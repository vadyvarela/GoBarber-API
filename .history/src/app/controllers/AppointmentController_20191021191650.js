...
import cancellationMails from '../jobs/cancellationMails';
  ...
  async store(req, res) {
    ...
    */
    ...
    if (req.userId === provider_id) {
      return res
        .status(400)
        .json({ error: 'You cant create appoitment with your self' });
    }
  ...
  async delete(req, res) {
    ...
    if (appointment.user_id !== req.userId) {
      return res.status(401).json({
        error: 'You dont have permition to cancel this appointment',
      });
    }
    ...
    await Queue.add(cancellationMails.key, {
      appointment,
    });
